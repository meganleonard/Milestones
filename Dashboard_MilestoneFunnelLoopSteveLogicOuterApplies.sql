...
SELECT DISTINCT
	...
	(SELECT DISTINCT 
		...
		FROM 
			...
			WHERE 
			TermID_MIS4 > CONCAT(2,@counter,6)
	) ce 



----------------------------------------------------------------------------------------------------------------------
-- Create #ay table, pulls academic year 
		...
		(SELECT
			...
			FROM 
				...
				WHERE
					...
					ce.SUCCESS = 1
					AND 
					ce.TermID_MIS4 > CONCAT(2,@counter,6)
					AND 
					ce.TermID_MIS4 <= b.TermID_MIS4 
					AND
					ce.stc_person_id = b.STC_PERSON_ID
	...
	FROM
		...
		(SELECT
			...
			FROM 
				--#tmte t
				--LEFT JOIN 
				datatel.dbo.FACTBOOK_CoreEnrollment_View ce --ON ce.STC_PERSON_ID = t.STC_PERSON_ID AND ce.TermID_MIS4 = t.TermID_MIS4
				WHERE
				ce.TermID_MIS4 > CONCAT(2,@counter,6)
				--and
				--ce.TermID_MIS4 <=t.TermID_MIS4
				--and STC_PERSON_ID = '0367778'
	...
	FROM 
		(SELECT DISTINCT
			...
			FROM 
				...
				AND ce.TermID_MIS4 > CONCAT(2,@counter,6)
		) a
		LEFT JOIN 
...
SELECT * FROM #Final WHERE STC_PERSON_ID = '0367778'  order by 2,3

select stc_term, TermID_MIS4, stc_person_id, STC_COURSE_NAME, stc_cred, STC_CMPL_CRED from datatel.dbo.FACTBOOK_CoreEnrollment_View  where STC_PERSON_ID = '0367778' order by 2
...
select * from pro.dbo.Dashboard_Funnel_Milestones
